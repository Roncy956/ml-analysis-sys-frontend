...
import {getForestData} from "@/api/result.js";
...
onMounted(async () => {
    ...
    const res = await getForestData()
    if (res.code === 1) {
        feature.value = res.data.feature_importance.map(item => item.feature).slice(0, 10).reverse();
        importance.value = res.data.feature_importance.map(item => item.importance).slice(0, 10).reverse();
        predictList.value = res.data.plot_data.actual_vs_predicted.predicted;
        actualList.value = res.data.plot_data.actual_vs_predicted.actual;
        residualsList.value = res.data.plot_data.residuals.residuals;
        residualsPredictedList.value = res.data.plot_data.residuals.predicted;
